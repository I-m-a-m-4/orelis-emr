...
import { updatePatientAction, saveEncounterAction } from '@/app/actions';
...
function EditEncounterForm({ encounter, patient, clinic }: EditEncounterFormProps) {
    const router = useRouter();
    const { toast } = useToast();
    const [state, formAction] = useActionState(saveEncounterAction, { isSuccess: false, message: '', errors: undefined });
    ...
    const [spo2, setSpo2] = useState(encounter.vitals?.find(v => v.type === 'spo2')?.value || '');
    ...
    return (
        <form action={formAction} className="flex flex-col gap-6 w-full">
            ...
            <input type="hidden" name="patientId" value={patient.id} />
            <input type="hidden" name="patientName" value={`${patient.firstName} ${patient.surname}`} />
            <input type="hidden" name="doctorId" value={encounter.doctorId} />
            <input type="hidden" name="doctorName" value={encounter.doctorName} />
            <input type="hidden" name="clinicId" value={patient.clinicId} />
            <input type="hidden" name="date" value={encounter.date} />
            <input type="hidden" name="vitalsJson" value={JSON.stringify(vitalsList)} />
            <input type="hidden" name="prescriptionsJson" value={JSON.stringify(prescriptions)} />
        ...
        <div className="flex flex-col gap-6 noisy-bg pb-20">
            ...
            {currentPage === 1 ? (
                ...
                (() => {
                    ...
                    if (!enc) return null;
                    return <EditEncounterForm encounter={enc} patient={patient} clinic={clinic} />;
                })()
            )}
